...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=OPENSTRIKE_3DS_MAPS");
    println!("cargo:rerun-if-env-changed=OPENSTRIKE_3DS_DATA_ROOT");

    if env::var_os("CARGO_FEATURE_EMBEDDED_MAP_CATALOG").is_none() {
        return;
    }

    let names_source = required_env("OPENSTRIKE_3DS_MAPS");
    let names: Vec<&str> = names_source.split(',').collect();
    if names.len() != MAP_COUNT {
        panic!("OPENSTRIKE_3DS_MAPS must contain exactly {MAP_COUNT} comma-separated map names");
    }
    for name in &names {
        if !safe_map_name(name) {
            panic!(
                "OPENSTRIKE_3DS_MAPS contains unsafe name {name:?}; \
                 use lowercase ASCII letters, digits, '_' or '-'"
            );
        }
    }
    if names.windows(2).any(|pair| pair[0] >= pair[1]) {
        panic!("OPENSTRIKE_3DS_MAPS must be strictly sorted with no duplicates");
    }

    let root_source = required_env("OPENSTRIKE_3DS_DATA_ROOT");
    let root = root_source.trim_end_matches(['/', '\\']);
    if root.is_empty() || !root.is_ascii() || root.as_bytes().contains(&0) {
        panic!("OPENSTRIKE_3DS_DATA_ROOT must be a non-empty NUL-free ASCII path");
    }

    let mut generated = String::from(
        "// @generated by crates/openstrike-3ds/build.rs\n\
    ...
    let output = PathBuf::from(required_env("OUT_DIR")).join("3ds_map_catalog.rs");
    fs::write(output, generated).expect("write generated 3DS map catalogue");
}
