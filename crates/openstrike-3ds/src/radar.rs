...
pub fn raster(map: &CookedMap<'_>, floor: f32) -> (Projection, Vec<u8>) {
    let p = Projection::new(map.bounds.0, map.bounds.1);
    let mut heights = vec![f32::NEG_INFINITY; WIDTH * HEIGHT];
    for batch in &map.batches {
        let indices = &map.indices[batch.index_base as usize..][..batch.index_count as usize];
        for tri in indices.chunks_exact(3) {
            ...
            // GoldSrc surface loops are clockwise viewed from the empty side.
            ...
            for y in miny..=maxy {
                for x in minx..=maxx {
                    ...
                    let score = |height: f32| {
                        (height - floor).abs() + if height > floor + 40. { 512. } else { 0. }
                    };
