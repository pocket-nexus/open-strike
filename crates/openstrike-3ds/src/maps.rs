//! App-private cooked-map loading through the 3DS SD filesystem.
//!
...
include!(concat!(env!("OUT_DIR"), "/3ds_map_catalog.rs"));
...
impl AlignedMapBuffer {
    ...
    pub(crate) unsafe fn load(&mut self, entry: &MapCatalogEntry) -> Result<&[u8], &'static str> {
        ...
        // Cooked P3D sections require a 16-byte base. The C allocator only
        ...
        self.len = 0;
        self.words
            .try_reserve(words.saturating_sub(self.words.len()))
            .map_err(|_| "not enough memory for map")?;
